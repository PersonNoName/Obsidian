...
flowchart TD
    ...
    subgraph Harness["AgentHarness 编排层"]
        Boundary["Public API Boundary<br/>公共入口与阶段控制"]
        RuntimeState["Harness Config / Turn Snapshot<br/>运行时配置与本轮快照"]
        Queues["Runtime Queues<br/>运行期队列"]
        Events["Hooks & Events<br/>事件与 Hook 通道"]
        Pending["PendingSessionWrites<br/>待持久化写入"]
    end

    subgraph SessionLayer["Session / Storage 层"]
        Session["Session<br/>会话上下文与会话树"]
        Storage["SessionStorage / JSONL Tree<br/>持久化存储"]
    end

    subgraph LoopLayer["Low-level Agent Loop<br/>低层 Agent Loop"]
        Loop["runAgentLoop<br/>Turn 执行引擎"]
        ToolFlow["Tool Call Flow<br/>工具调用流程"]
        LoopEvents["AgentEvent<br/>Loop 生命周期事件"]
    end

    subgraph ProviderLayer["Provider / Stream 层"]
        Provider["Model Provider<br/>模型 Provider"]
        Stream["StreamFn / streamSimple<br/>流式传输"]
        Auth["Auth / StreamOptions<br/>认证与请求选项"]
    end

    subgraph SupportLayer["Support Capabilities<br/>支撑能力"]
        Env["ExecutionEnv<br/>执行环境"]
        Resources["AgentHarnessResources<br/>技能与提示模板资源"]
        Tools["AgentTool Registry<br/>工具集合"]
        StructOps["Compaction / Tree Navigation<br/>压缩与会话树导航"]
    end
